    ...
    <React.Fragment>
      <section className={classes.root}>
        <div {...getRootProps({ className: 'dropzone' })}>
          <input {...getInputProps()} multiple name="imageData" encType="multipart/form-data" onSubmit={handleChange} />
          <UploadIcon className="iconSmall" />
          <p>Dra og slipp filer her, eller klikk for å velge fil(er)</p>
        </div>
        <aside style={thumbsContainer}>{thumbs}</aside>
        {progress ? (
          <LinearProgressWithLabel autohideduration={3000} className={classes.borderProgress} value={progress} />
        ) : null}
        <p>
          <Button
